...
export default function QuestionsPage() {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const writeLine = (text: string, size: number, style: 'normal' | 'bold' = 'normal', color: [number, number, number] = [30, 30, 30]) => {
      ...
      doc.text(
        `Exported ${format(new Date(), 'dd MMM yyyy, HH:mm')}  •  ${questions.length} question${questions.length !== 1 ? 's' : ''}${debouncedSearch ? `  •  Filter: "${debouncedSearch}"` : ''}`,
        margin, 22
      );
      y = 36;
      ...
      questions.forEach((q, idx) => {
        ...
        // Subject / Chapter in grey on right
        ...
        if (q.options) {
          ...
          if (opts.length > 0) {
            ...
            y += 4.5;
            opts.forEach((opt) => {
              writeWrapped(opt, 9, 'normal', [50, 50, 50], 4);
            });
          }
        }
        ...
        const footerParts = [
          `Score: ${q.qualityScore ?? '—'}/10`,
          q.modelUsed ? `Model: ${q.modelUsed}` : null,
          q.generatedAt ? format(parseISO(q.generatedAt), 'dd MMM yyyy') : null,
        ].filter(Boolean).join('  •  ');
        doc.text(footerParts, margin, y);
        y += 5;
